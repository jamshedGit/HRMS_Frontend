...
export function FormPage({ history }) {
  ...
  const FormUIEvents = {
    ...
    openDeleteFormDialog: (id, status) => {
      history.push(`/employee_profile/read-all-employee-profile/${id}/${status}/delete`);
    },
  ...
  return (
    ...
    <FormUIProvider FormUIEvents={FormUIEvents}>
      ...
      <Route path="/employee_profile/read-all-employee-profile/:id/:status/delete">
        {({ history, match }) => (
          <FormDeleteDialog
            show={match != null}
            ...
            onHide={() => {
              history.push("/employee_profile/read-all-employee-profile");
            }}
          />
        )}
      </Route>
