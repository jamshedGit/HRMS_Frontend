...
export function FormTable() {
  ...
  const { currentState, userAccess } = useSelector(
    (state) => {
      return {
        currentState: state.attendance_configuration,
  ...
  useEffect(() => {
    FormUIProps.setIds([]);
    dispatch(actions.fetchAttendanceConfiguration(FormUIProps.queryParams));
  }, [FormUIProps.queryParams, dispatch, totalCount]);

  const isAccessForEdit = userAccess?.find(
    (item) => item.componentName === "UpdateAttConfiguration"
  );

  const isAccessForDelete = userAccess?.find(
    (item) => item.componentName === "DeleteAttConfiguration"
  );
  // Table columns
  const columns = [
