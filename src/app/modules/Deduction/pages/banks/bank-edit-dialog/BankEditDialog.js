...
export function BankEditDialog({ id, show, onHide, userForRead }) {
  ...
  const {
    ...
    userForEdit: state.deduction.userForEdit,
    roles: state.users.roles,
    centers: state.users.centers,
    userStatusTypes: state.users.userStatusTypes,
    isuserForRead: state.deduction.userForRead,
  }));
  ...
  const saveIncident = async (user) => {
    ...
    if (!id) {
      ...
      await dispatch(actions.createDeduction(user, disbaleLoading, onHide));
      ...
      const deductionUpdatedFields = {
        Id: user.Id,
        deductionCode: user.deductionCode,
        deductionName: user.deductionName,
      ...
      console.log("skill updated", deductionUpdatedFields);
      await dispatch(actions.updatedDeduction(deductionUpdatedFields, disbaleLoading, onHide));
      await dispatch(actions.fetchUsers(usersUIProps.queryParams));
    }
  };
