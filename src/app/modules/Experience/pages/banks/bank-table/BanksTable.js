...
export function BanksTable() {
  ...
  const { currentState, userAccess } = useSelector(
    (state) => {  console.log("academic state ",state); return {
      ...
      currentState: state.experience,
      userAccess: state?.auth?.userAccess["Experience"],
    }},
    shallowEqual
  );
  ...
  const isAccessForEdit = userAccess?.find(
    (item) => item.componentName === "UpdateExperience"
  );

  const isAccessForDelete = userAccess?.find(
    (item) => item.componentName === "DeleteExperience"
  );
  // Table columns
  const columns = [
    // {
