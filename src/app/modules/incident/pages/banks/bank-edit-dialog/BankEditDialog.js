...
export function BankEditDialog({ id, show, onHide, userForRead }) {
  ...
  const {
    ...
    userForEdit: state.incident.userForEdit,
    roles: state.users.roles,
    centers: state.users.centers,
    userStatusTypes: state.users.userStatusTypes,
    isuserForRead: state.incident.userForRead,
  }));
  ...
  const saveIncident = async (user) => {
    ...
    if (!id) {
     ...
     await dispatch(actions.createincident(user, disbaleLoading, onHide));
     await dispatch(actions.fetchUsers(usersUIProps.queryParams));
      ...
      const skillUpdatedFields = {
        Id: user.Id,
        incidentDetail: user.incidentDetail,
     ...
     await dispatch(actions.updatedincident(skillUpdatedFields, disbaleLoading, onHide));
     await dispatch(actions.fetchUsers(usersUIProps.queryParams));
    }
  };
