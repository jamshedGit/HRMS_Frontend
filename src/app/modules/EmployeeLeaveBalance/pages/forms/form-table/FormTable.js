...
export function FormTable() {
  ...
  const { currentState, userAccess } = useSelector(
    (state) => {
      return {
        currentState: state.Employee_Leave_Balance,
        userAccess: state?.auth?.userAccess["Employee_Leave_Balance"],
      }
    },
    shallowEqual
  );
  ...
  useEffect(() => {
    FormUIProps.setIds([]);
    dispatch(actions.fetchEmployeeLeaveBalance(FormUIProps.queryParams));
  }, [FormUIProps.queryParams, dispatch, totalCount]);

  const isAccessForEdit = userAccess?.find(
    (item) => item.componentName === "UpdateEmployeeLeaveBalance"
  );

  const isAccessForDelete = userAccess?.find(
    (item) => item.componentName === "DeleteEmployeeLeaveBalance"
  );
  // Table columns
  const columns = [
    {
