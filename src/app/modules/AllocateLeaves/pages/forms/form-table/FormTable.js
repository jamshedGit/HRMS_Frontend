...
export function FormTable() {
  ...
  const { currentState, userAccess } = useSelector(
    (state) => {
      return {
        currentState: state.Allocate_Leaves,
  ...
  useEffect(() => {
    FormUIProps.setIds([]);
    dispatch(actions.fetchAllocateLeaves(FormUIProps.queryParams));
  }, [FormUIProps.queryParams, dispatch, totalCount]);

  const isAccessForEdit = userAccess?.find(
    (item) => item.componentName === "UpdateAllocateLeaves"
  );

  const isAccessForDelete = userAccess?.find(
    (item) => item.componentName === "DeleteAllocateLeaves"
  );
  // Table columns
  const columns = [
    {
