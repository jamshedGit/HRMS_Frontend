...
export function BanksTable() {
  ...
  const { currentState, userAccess } = useSelector(
    (state) => {  console.log("academic state ",state); return {
      ...
      currentState: state.employee_salary_earning,
      userAccess: state?.auth?.userAccess["Employee_Salary_Earning"],
    }},
    shallowEqual
  );
  ...
  const isAccessForEdit = userAccess?.find(
    (item) => item.componentName === "UpdateEmployeeSalaryEarning"
  );

  const isAccessForDelete = userAccess?.find(
    (item) => item.componentName === "DeleteEmployeeSalaryEarning"
  );
  // Table columns
  const columns = [
    // {
