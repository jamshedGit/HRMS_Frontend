...
export function BanksTable() {
  ...
  const { currentState, userAccess } = useSelector(
    (state) => {  console.log("academic state ",state); return {
      ...
      currentState: state.salary_expatriate,
      userAccess: state?.auth?.userAccess["Salary_Expatriate"],
    }},
    shallowEqual
  );
  ...
  const isAccessForEdit = userAccess?.find(
    (item) => item.componentName === "UpdateEmployeeSalaryExpatriate"
  );

  const isAccessForDelete = userAccess?.find(
    (item) => item.componentName === "DeleteEmployeeSalaryExpatriate"
  );
  // Table columns
  const columns = [
    // {
    ...
    {
      dataField: "visaSponsorshipStatus",
      text: "Visa Sponsorship Status",
      sort: false,
      sortCaret: sortCaret,
      headerSortingClasses,
      style: {
    ...
    {
      dataField: "airTicketAmount",
      text: "Air Ticket Amount",
      sort: false,
      sortCaret: sortCaret,
      headerSortingClasses,
      style: {
