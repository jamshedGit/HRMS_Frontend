...
export function BankEditDialog({ id, show, onHide, userForRead }) {
  ...
  const {
    ...
    userForEdit: state.stoppage.userForEdit,
    roles: state.users.roles,
    centers: state.users.centers,
    userStatusTypes: state.users.userStatusTypes,
    isuserForRead: state.stoppage.userForRead,
  }));
  ...
  const saveIncident = async (user) => {
    ...
    if (!id) {
      ...
      await dispatch(actions.createStoppageAllowance(user, disbaleLoading, onHide));
      ...
      const stoppage_allowanceUpdatedFields = {
        Id: user.Id,
        earning_deduction_type : user.earning_deduction_type,
