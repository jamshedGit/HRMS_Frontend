...
export function BankEditDialog({ id, show, onHide, userForRead }) {
  ...
  const {
    ...
    userForEdit: state.loan_type.userForEdit,
    roles: state.users.roles,
    centers: state.users.centers,
    userStatusTypes: state.users.userStatusTypes,
    isuserForRead: state.loan_type.userForRead,
  }));
  ...
  const saveLoanType = async (user) => {
    ...
    if (!id) {
      ...
      await dispatch(actions.createLoanType(user, disbaleLoading, onHide));
