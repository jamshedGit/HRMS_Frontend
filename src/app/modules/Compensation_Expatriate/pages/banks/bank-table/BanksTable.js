...
export function BanksTable() {
  ...
  const { currentState, userAccess } = useSelector(
    (state) => {  console.log("academic state ",state); return {
      ...
      currentState: state.compensation_expatriate,
      userAccess: state?.auth?.userAccess["Compensation_Expatriate"],
    }},
    shallowEqual
  );
  ...
  const isAccessForEdit = userAccess?.find(
    (item) => item.componentName === "UpdateCompensationExpatriate"
  );

  const isAccessForDelete = userAccess?.find(
    (item) => item.componentName === "DeleteCompensationExpatriate"
  );
  // Table columns
  const columns = [
    // {
