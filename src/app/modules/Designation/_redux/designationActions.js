...
import { designationSlice, callTypes } from "./designationSlice";
import { toast } from "react-toastify";

const { actions } = designationSlice;
...
export const fetchUsers = (queryparm) => async (dispatch) => {
  ...
  return requestFromServer.getAllDesignation(queryparm)
...
export const fetchUser = (id) => (dispatch) => {
  ...
  if (!id) {
    return dispatch(actions.designationFetchedForEdit({ userForEdit: undefined }));
  }
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(actions.designationFetchedForEdit({ userForEdit: entities }));
    })
    .catch((error) => {
...
export const deleteDesignation = (id) => (dispatch) => {
  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      dispatch(actions.designationDeleted({ Id: id }));
...
export const createDesignation = (religionForCreation, disbaleLoading, onHide) => (
  dispatch
) => {
  ...
  return requestFromServer
    .createDesignation(religionForCreation)
    .then((res) => {
      dispatch(actions.startCall({ callType: callTypes.action }));
      const user = res.data?.data;
      console.log("religion data");
      console.log(user);
      dispatch(actions.designationCreated(user));
      disbaleLoading();
...
export const updateDesignation = (user, disbaleLoading, onHide) => (dispatch) => {
  return requestFromServer
    .updateDesignation(user)
    .then((response) => {
      ...
      dispatch(actions.designationUpdated({ updatedReligion }));
      dispatch(actions.startCall({ callType: callTypes.action }));
      disbaleLoading();
      onHide();
